...
Feature: Create User Group
  ...
  Scenario: Attempt to create a user group with a duplicate name
    Given the following user group details:
      | name        | description  | productId  | institutionId                         | status  | members                                                                   |
      | io group    | TestGroup    | prod-io    | 9c8ae123-d990-4400-b043-67a60aff31bc  | ACTIVE  | 525db33f-967f-4a82-8984-c606225e714a,a1b7c86b-d195-41d8-8291-7c3467abfd30 |
    When I send a POST request to "/v1/user-groups" with the given details
  ...
  Scenario: Attempt to create a user group without authentication
    ...
    When I send a POST request to "/v1/user-groups" with the given details without authentication
